...
// 生成单位半球随机向量
inline VecN<3> randomUnitHemisphere() {
...
void Graphic::SSAO(
    ...
    for (auto y = 0; y < h; y++) {
        for (auto x = 0; x < w; x++) {
            ...
            const VecN<4>& worldPos = Gdata[idx].worldPosi; // 从Gdata中获取世界坐标
            auto viewPos = ViewMat * worldPos;
            auto& worldnormal = Gdata[idx].normal; // 从Gdata中获取法线
            auto viewnormaltmp = ViewMat * VecN<4>{worldnormal[0], worldnormal[1], worldnormal[2], 0};
            auto normal = normalize(VecN<3>{viewnormaltmp[0], viewnormaltmp[1], viewnormaltmp[2]});

            // 构建 TBN
            VecN<3> randvec = normalize(randomNoiseVector());
            VecN<3> tangent = normalize(randvec - normal * dot(randvec, normal));
            MatMN<3, 3> TBN = MatMN<3, 3>(tangent, cross(normal, tangent), normal).Transpose();
            int count = 0;
            for (int i = 0; i < sampleCount; i++) {
                VecN<3> randomVec = TBN * randomUnitHemisphere();
                VecN<4> tmp = {randomVec[0], randomVec[1], randomVec[2], 0.f};
                const VecN<4> randomPos = viewPos + tmp * radius;
                VecN<4> rclipPos = Projection * randomPos;
                VecN<4> rndcPos = rclipPos / rclipPos[3];
